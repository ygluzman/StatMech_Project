...
def lamellar_test(polymer1, polymer2, charges, temp, field=1):
    """
    Evalutes if move (reptation or pivot ) will be accepted in a lamellar field in the y direction
    ...
    field -- strength of lamellar field (default 1)
    
    Returns:
    1) Original or accepted polymer
    2) Energy of that polymer

    """
    T=temp

    #Calculate energies and change in energy:

    n = len(polymer1)
    E1 =0
    for i in range(0,n):
        E1 += charges[i] * field * polymer1[i][1]

    E2 =0
    for i in range(0,n):
        E2 += charges[i] * field * polymer2[i][1]
